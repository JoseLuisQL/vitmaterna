 ...
 * carga de mensajes antiguos y el auto-scroll al final.
 *
...
export function MessageThread({
  ...
  // Intercala separadores de día entre los mensajes (CHAT-09).
  ...
  const renderRow = ({ item }: { item: Row }) => {
    ...
    return <Bubble msg={item.msg} currentUserId={currentUserId} mineColor={mineColor} onOpenContent={onOpenContent} />;
  };
  ...
  return (
    <FlatList
      ...
      ListFooterComponent={otherTyping ? <View style={styles.typingWrap}><TypingDots color={commonColors.textSecondary} /></View> : null}
      ListEmptyComponent={
...
function Bubble({ msg, currentUserId, mineColor, onOpenContent }: BubbleProps): React.ReactElement {
  ...
  if (msg.tipo === 'educacion' && msg.content) {
    ...
    const CIcon = cm.icon;
    return (
      <View style={[styles.bubble, isMe ? [styles.mine, { backgroundColor: mineColor }] : styles.other, styles.eduBubble]}>
        {!!msg.text && (
          <Text style={[styles.text, isMe ? styles.textMine : styles.textOther, { marginBottom: spacing.sm }]}>{msg.text}</Text>
        )}
        <View
          style={[styles.eduCard, !!onOpenContent && ({ cursor: 'pointer' } as any)]}
          onTouchEnd={onOpenContent && msg.content ? () => onOpenContent(msg.content!.id) : undefined}
        >
  ...
  return (
    <View style={[styles.bubble, isMe ? [styles.mine, { backgroundColor: mineColor }] : styles.other]}>
      {isImage ? (
        <Image
